  ...
  module Allowables
    module Generators
      class PermissionGenerator < ActiveRecord::Generators::Base
        ...
        namespace "allowables:permission"

        include ::Allowables::Generators::OrmHelpers
        source_root File.expand_path("../templates", __FILE__)

        def copy_allowables_migration
          if (behavior == :invoke && model_exists?) || (behavior == :revoke && migration_exists?(:permission, table_name))
            migration_template "permission_existing.rb", "db/migrate/add_allowables_permission_to_#{table_name}"
          else
            migration_template "permission.rb", "db/migrate/allowables_permission_create_#{table_name}"
          end
        end
