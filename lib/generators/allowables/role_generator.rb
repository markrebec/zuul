      ...
      class RoleGenerator < ActiveRecord::Generators::Base
        ...
        def copy_allowables_migration
          if (behavior == :invoke && model_exists?) || (behavior == :revoke && migration_exists?(:role, table_name))
            migration_template "role_existing.rb", "db/migrate/add_allowables_role_to_#{table_name}"
          else
            migration_template "role.rb", "db/migrate/allowables_role_create_#{table_name}"
          end
        end
